...
import 'package:carepetsapp/pages/viewLostPet.dart';
...
class LostPetsList extends StatefulWidget {
  @override
  _LostPetsListState createState() => _LostPetsListState();
}

class _LostPetsListState extends State<LostPetsList> {
  //Variables
  ...
  buildLostPetsResults() {
    Future<QuerySnapshot> lostsPets =
        lostPetsRef.orderBy('timeStamp', descending: true).get();

    setState(() {
      searchResultsFuture = lostsPets;
    });
    ...
    List<LostPetsResult> addResult(
        List<LostPetsResult> auxResults, List<LostPetsResult> searchResults) {
    ...
    List<LostPetsResult> compareFilterType(List<bool> boolList,
        List<LostPetsResult> searchResults, List<LostPetsResult> auxResults) {
      for (var k = 0; k < boolList.length; k++) {
        if (boolList[k] == true) {
          for (var l = 0; l < searchResults.length; l++) {
            if (searchResults[l].lostPet.type == typeListAnimals[k]) {
    ...
    List<LostPetsResult> compareFilterGender(List<bool> boolList,
        List<LostPetsResult> searchResults, List<LostPetsResult> auxResults) {
      for (var m = 0; m < boolList.length; m++) {
        if (boolList[m] == true) {
          for (var n = 0; n < searchResults.length; n++) {
            if (searchResults[n].lostPet.gender == genderListAnimals[m]) {
    ...
    List<LostPetsResult> compareFilterProvince(List<bool> boolList,
        List<LostPetsResult> searchResults, List<LostPetsResult> auxResults) {
      for (var o = 0; o < boolList.length; o++) {
        if (boolList[o] == true) {
          for (var p = 0; p < searchResults.length; p++) {
            if (searchResults[p].lostPet.province == provinceListAnimals[o]) {
    ...
    return FutureBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        List<LostPetsResult> searchResults = [];
        List<LostPetsResult> searchAuxResults = [];
        List<LostPetsResult> typeResults = [];
        List<LostPetsResult> genderResults = [];
        List<LostPetsResult> provinceResults = [];
...
//add types filter
        ...
        for (var doc in snapshot.data!.docs) {
          ...
          LostPetsResult searchResult = LostPetsResult(lostPet);
  ...
  Widget build(context) {
    return Scaffold(
      ...
      body: buildLostPetsResults(),
    );
  }
}

class LostPetsResult extends StatefulWidget {
  final LostPet lostPet;

  const LostPetsResult(this.lostPet);

  @override
  State<LostPetsResult> createState() => _LostPetsResultState();
}

class _LostPetsResultState extends State<LostPetsResult> {
  @override
      ...
      child: InkWell(
        onTap: () {
          Navigator.push(
              context,
              MaterialPageRoute(
                  builder: (context) => ViewLostPet(
                        lostPetId: widget.lostPet.id,
                      )));
        },
        child: Column(
                ...
                Padding(
                  ...
                  child: AutoSizeText(
                    widget.lostPet.name +
                        "\n\n" +
                        "Sexo: " +
                        widget.lostPet.gender +
                        "\n\nUbicación: " +
                        widget.lostPet.location,
                    style: const TextStyle(fontSize: 15.0),
                  ),
                ),
                if (widget.lostPet.type == "Perro")
                  Image.asset(
                    'assets/images/dog.png',
                    scale: 8,
                  ),
                if (widget.lostPet.type == "Gato")
                  Image.asset(
                    'assets/images/cat.png',
                    scale: 5,
                  ),
                if (widget.lostPet.type == "Conejo")
                  Image.asset(
                    'assets/images/rabbit.png',
                    scale: 10,
                  ),
                if (widget.lostPet.type == "Hamster")
                  Image.asset(
                    'assets/images/hamster.png',
                    scale: 7,
                  ),
                if (widget.lostPet.type == "Hurón")
                  Image.asset(
                    'assets/images/ferret.png',
                    scale: 6,
                  ),
                if (widget.lostPet.type == "Cobaya")
                  Image.asset(
                    'assets/images/guineaPig.png',
                    scale: 6,
                  ),
                if (widget.lostPet.type == "Chinchilla")
                  Image.asset(
                    'assets/images/chinchilla.png',
                    scale: 18,
                  ),
                if (widget.lostPet.type == "Pájaro")
                  Image.asset(
                    'assets/images/bird.png',
                    scale: 40,
                  ),
                if (widget.lostPet.type == "Pez")
                  Image.asset(
                    'assets/images/fish.png',
                    scale: 20,
                  ),
                if (widget.lostPet.type == "Tortuga")
                  Image.asset(
                    'assets/images/tortoise.png',
                    scale: 6,
                  ),
                if (widget.lostPet.type == "Erizo")
                  Image.asset(
                    'assets/images/hedgehog.png',
                    scale: 6,
                  ),
                if (widget.lostPet.type == "Caballo")
                  Image.asset(
                    'assets/images/horse.png',
                    scale: 10,
                  ),
                if (widget.lostPet.type == "Otro") const Icon(Icons.pets)
              ],
            ),
          ],
        ),
