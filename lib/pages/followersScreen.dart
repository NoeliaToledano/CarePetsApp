...
class _FollowersScreenState extends State<FollowersScreen> {
  ...
  buildFollowersResults() {
    Future<QuerySnapshot> followers =
        followersRef.doc(widget.userId).collection('userFollowers').get();
    setState(() {
    ...
    return FutureBuilder<QuerySnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          List<FollowersScreenResult> searchResults = [];
          for (var doc in snapshot.data!.docs) {
            User user = User.fromDocument(doc);
            if (widget.userId != user.id) {
              FollowersScreenResult searchResult = FollowersScreenResult(user);
...
class FollowersScreenResult extends StatelessWidget {
  final User user;
  const FollowersScreenResult(this.user);
